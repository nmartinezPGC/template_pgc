...
export class ModalLinkComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    this.findByIdActividadRecurso(this.modalidlinkk);
    this.tipoService();
    this.tiposService();
...
* Objetivo: closeModal cierra la modal activa
* Autor: Allan Madrid
****************************************************************************/
  closeModal() {
...
private UpdateLink() {
  ...
  // Ejecutamos el Recurso del EndPoint
  this._servicerecursosproyectoService.LinkUpdate(this._linkmodal1, this.modalidlinkk).subscribe(
    response => {
      if (response.status !== 200) {
